const data = `nine92jnhgqzctpgbcbpz
sevensddvc73three
9986fmfqhdmq8
7onexmxbzllfqb
six777
...
// concat first and last number in a row to a 2-digit number
// sum all 2-digit numbers

function isNumber(d: any) {
...
function partOne(lines: string[]) {
    ...
    lines.forEach(l => {
        ...
        const first: string = lineChars.find(d => isNumber(d))
        const last: string = lineChars.findLast(d => isNumber(d))
...
console.log(`result part 1: ${resultPartOne}`)
...
export function partTwo(lines: string[]) {
    ...
    function isStringNumber(d: string) {
        const num = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
        // 
        if (d.length === 1) {
            // num.map(n => n.substring(0,1)).includes(d)
            return "otfsen".includes(d)
        }
        else if (d.length === 2) {
            return ["on", "tw", "th", "fo", "fi", "si", "se", "ei", "ni"].includes(d)
        } else if (d.length === 3) {
            if (["one", "two", "six"].includes(d)) {
                return numMap.get(d)
            } else {
                return ["thr", "fou", "fiv", "sev", "eig", "nin"].includes(d)
            }
        } else if (d.length === 4) {
            if (["four", "five", "nine"].includes(d)) {
                return numMap.get(d)
            } else {
                return ["thre", "seve", "eigh"].includes(d)
            }
        } else if (d.length === 5) {
            return numMap.get(d)
        }
    ...
    function isStringNumberRev(d: string) {
        const num = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
        // 
        if (d.length === 1) {
            // num.map(n => n.substring(0,1)).includes(d)
            return "eorxnt".includes(d)
        }
        else if (d.length === 2) {
            return ["ne", "wo", "ee", "ur", "ve", "ix", "en", "ht", "ne"].includes(d)
        } else if (d.length === 3) {
            if (["one", "two", "six"].includes(d)) {
                return numMap.get(d)
            } else {
                return ["ree", "our", "ive", "ven", "ght", "ine"].includes(d)
            }
        } else if (d.length === 4) {
            if (["four", "five", "nine"].includes(d)) {
                return numMap.get(d)
            } else {
                return ["hree", "even", "ight"].includes(d)
            }
        } else if (d.length === 5) {
            return numMap.get(d)
        }
    ...
    lines.forEach(l => {
        ...
        for (let i = 0; i < l.length; i++) {
            ...
            numAccumulator += l[i];
            const isn = isStringNumber(numAccumulator)
            if (!isn) numAccumulator = numAccumulator.substring(1)
            if (isn && isNumber(isn)) {
                first = isn;
                break;
            }
        ...
        for (let i = l.length - 1; i >= 0; i--) {
            ...
            numAccumulator = l[i] + numAccumulator;
            const isn = isStringNumberRev(numAccumulator)
            if (!isn) {
                numAccumulator = numAccumulator.substring(0, numAccumulator.length - 1)
            }
            if (isn && isNumber(isn)) {
                last = isn;
                break;
        ...
        const num = parseInt(first + last);
        if (isNaN(num)) throw Error(`${num} - ${l}`)
        return sum += num
    })
    return sum
}
